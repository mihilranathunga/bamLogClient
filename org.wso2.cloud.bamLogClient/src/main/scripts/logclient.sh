...
start() {
  ...
  if [ $pid -ne 0 ] ; then
    echo "BAM logging THRIFT client : Already started"
    exit 1
  fi
...
stop() {
  ...
  if [ $pid -eq 0 ] ; then
    echo "BAM logging THRIFT client: Already stopped"
    exit 1
  fi
...
status() {
  ...
  if [ $? -ne 0 ] ; then
    echo "BAM logging THRIFT client: Running"
  else
    echo "BAM logging THRIFT client: Stopped"
  fi
...
version() {
echo "BAM logging THRIFT client v0.1"
}
